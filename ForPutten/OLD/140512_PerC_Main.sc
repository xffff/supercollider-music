		...
		\amp, 
			Pif(Pbinop('==', Pkey(\midinote), 60), Pexprand(0.75,1.0,inf),
				...
				Pif(Pbinop('==', Pkey(\midinote), 66), Pexprand(0.75,1.0,inf), 
					Pwrand([0,Pexprand(0.5,1.0,1)],[1,24].normalizeSum,inf)))
			)
	), 0)
);
);
...
Pdef(\section2_drum1, Pfindur(~section2_dur, Pbind(
		...
		\amp, Pseq([1.0,
				Pif(Pbinop('==', Pkey(\midinote), 48), Pwrand([Pn(0,1),Pexprand(0.75,0.9,1)],{1.0.rand}.dup.normalizeSum,inf),
					Pif(Pbinop('==', Pkey(\midinote), 50), Pwrand([Pn(0,1),Pexprand(0.75,0.9,1)],{1.0.rand}.dup.normalizeSum,inf),
					Pif(Pbinop('==', Pkey(\midinote), 52), Pwrand([Pn(0,1),Pexprand(0.75,0.9,1)],{1.0.rand}.dup.normalizeSum,inf),
						Pwrand([Pn(0,1),Pexprand(0.75,0.9,1)],[0.8,0.2].normalizeSum,inf)))
				)
			],1),
...
Pdef(\section2_fx3, Pfindur(~section2_dur, Pbind(
		...
		\sus, Pkey(\dur),
		\rel, Pkey(\dur)*2,
		\amp, Pexprand(0.3,0.75,inf), // decrescendo
		\freq, Prand(~pelog,inf).max(1).midicps,
		\lagtime, Prand([Pn(0,inf),Prand([Pkey(\dur).rand,0],inf)],1), // can't decide... stochastic is good
...
Pdef(\section6_drum1, Pfindur(~section6_dur, Pbind(
		\type, \midi,\midicmd, \noteOn, 	
		\chan, 0,
		\midinote, Pn(Pshuf([41,43,45],Prand((1..10),1)),inf),
		\midiout, ~mOut[3],
		\dur, 1/4, 
...
Pdef(\section7_sy1, Pfindur(~section7_dur, Pbind(
		...
		\decay, 0.1,
		\amp, 1.5,
		\freq, {~pelog[exprand(~pelog.size*0.7,~pelog.size*0.9)].midicps}.dup(20), 
		\modmul, 0,
		\modfreq, {Array.series(10,0.1,0.01).choose}.dup(20),
		\amselect, 1,
		\amfreqline, 1,
		\amfreq, Array.fill(20, { |i| ~pelog[i]+1 }),
		\amfreqend, Array.fill(20, { |i| ~pelog[i]+1 }).reciprocal,
		\amfreqlinetime, ~section7_dur,
		\amenvrel, {exprand(0.01,0.1)}.dup(20),
	))
);
);
...
Pdef(\section7_fx1, Pfindur(~section7_dur, Pbind(
		...
		\windowSize, 4.0,
		\timeDispersion, Pexprand(0.01,Pseq([1.0,2.0,3.0,3.95],inf),1),
		\pitchDispersion, Pexprand(0.01,Pseq([1.0,2.0,3.0,3.95],inf),1)
	))
);
);
...
Pdef(\section7_fx3, Pfindur(~section7_dur, Pbind(
		...
		\in, ~master_fx_bus.subBus(2,1),
		\out, 12,
		\dur, ~section7_dur, \delta, ~section7_dur,
		\sus, ~section7_dur,
		\atk, 0.1,
		\rel, 20,
...
Pdef(\section8_drum1, Pfindur(~section8_dur, Pbind(
		...
		\midiout, ~mOut[3],
		\dur, 1/32, 
		\amp, Pseg([Pseq(Array.series(50,0.02,0.02),1),Pseq(Array.series(100,0.01,0.01).reverse,1)],Prand(Scale.pelog.degrees+1,inf),'exp')
	))
);
);
...
Pdef(\section8_fx1, Pfindur(~section8_dur, Pbind(
		...
		\rel, 20,
		\amp, 0.35,
		\warpfactor, (Array.fill(10, { |i| ~pelog[i]+2 })++Array.fill(10, { |i| ~pelog[i]+2 }).reciprocal).sort,
		\freqscale, Pkey(\warpfactor)
	))
);
);
...
Pdef(\section9_drum1, Pfindur(~section9_dur, Pbind(
		\type, \midi,\midicmd, \noteOn, 	
		\chan, 0,
		\midinote, Pseq([63,60],1),
		\midiout, ~mOut[0],
		\dur, Pseq([~section9_dur,1],1), 
		\amp, 0.5
	))
);
);
