...
fork{
	...
	~begin_playback = fork{
			...
			s.waitForBoot(
				...
				thisProcess.interpreter.executeFile(~path ++ "/PerC_Synths.sc");
				1.wait;
				postln("Audio Routing... ");
				thisProcess.interpreter.executeFile(~path ++ "/PerC_Main.sc");
				1.wait;
				postln("Loading Sequencer...");
				thisProcess.interpreter.executeFile(~path ++ "/PerC_Sequencer.sc");
				postln("Initialised!");
			);
